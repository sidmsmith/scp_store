...
// Load Opportunity Buys cards (summary page)
async function loadOpportunityBuysCards() {
  ...
  try {
    // Call plannedPurchase/search API
    ...
    // Render PlannedPurchase cards
    renderOpportunityBuyCards(plannedPurchases);
...
// Render PlannedPurchase cards (similar to renderOrderCards)
function renderOpportunityBuyCards(plannedPurchases) {
  if (!ordersContainer) return;
  
  ordersContainer.innerHTML = '';
  
  plannedPurchases.forEach((plannedPurchase, index) => {
    const purchaseCard = document.createElement('div');
    purchaseCard.className = 'order-card';
    
    // Extract purchase details
    const purchaseId = plannedPurchase.PlannedPurchaseId || `Opportunity Buy ${index + 1}`;
    const purchaseName = plannedPurchase.PlannedPurchaseName || '';
    const itemId = plannedPurchase.ItemId || '';
    const purchaseQuantity = plannedPurchase.PurchaseQuantity || '';
    const purchaseOnDate = plannedPurchase.PurchaseOnDate || '';
    const plannedReceiptDate = plannedPurchase.PlannedReceiptDate || '';
    const locationId = plannedPurchase.LocationId || storeId || '';
    
    purchaseCard.innerHTML = `
      <div class="order-card-header">
        <div>
          <h4 class="order-card-title">${purchaseName || `Opportunity Buy ${index + 1}`}</h4>
        </div>
      </div>
      <div class="order-card-details">
        ${itemId ? `<div class="order-card-detail-row">
          <span class="order-card-detail-label">Item ID:</span>
          <span class="order-card-detail-value">${itemId}</span>
        </div>` : ''}
        ${purchaseQuantity ? `<div class="order-card-detail-row">
          <span class="order-card-detail-label">Purchase Quantity:</span>
          <span class="order-card-detail-value">${formatNumber(purchaseQuantity)}</span>
        </div>` : ''}
        ${purchaseOnDate ? `<div class="order-card-detail-row">
          <span class="order-card-detail-label">Purchase On Date:</span>
          <span class="order-card-detail-value">${purchaseOnDate}</span>
        </div>` : ''}
        ${plannedReceiptDate ? `<div class="order-card-detail-row">
          <span class="order-card-detail-label">Planned Receipt Date:</span>
          <span class="order-card-detail-value">${plannedReceiptDate}</span>
        </div>` : ''}
      </div>
    `;
    
    // Store PlannedPurchase data for use in item details page
    purchaseCard.setAttribute('data-planned-purchase-id', purchaseId);
    purchaseCard.setAttribute('data-item-id', itemId);
    purchaseCard.setAttribute('data-location-id', locationId);
    
    // Add click handler to load item details
    purchaseCard.addEventListener('click', async () => {
      logToConsole(`Opportunity Buy card clicked: ${purchaseId}`, 'info');
      await loadOpportunityBuyItems(plannedPurchase);
    });
    
    ordersContainer.appendChild(purchaseCard);
  });
}

// Load Opportunity Buy items (when a card is clicked)
async function loadOpportunityBuyItems(plannedPurchase) {
  if (!token) {
    status('Please authenticate first', 'error');
    return;
  }
  ...
  try {
    ...
    // Step 4: Render cards (reuse renderOpportunityBuysCards but with adapted data)
